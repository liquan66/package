...
from xml.dom.minidom import Document  # 这个错误不用管
import numpy as np
import copy, cv2
"""
在运行这个文件之前需要将PNG图片转化为jpg图片-----运行PngToJpg.py文件
将图片DOTA数据集进行分割，将标注文件转化为voc格式
raw_data = 'F:/code/DATA/'  # 原数据集文件夹
raw_images_dir = os.path.join(raw_data, 'xin')  # xin文件夹中为jpg文件；
raw_label_dir = os.path.join(raw_data, 'DOTA15')  #原来文件夹的标注文件

save_dir = 'F:/code/DATA/DOTA_clip3/'  # 裁剪后的图片和标注文件的地址，需要在此地址下建立：images（保存图片）和labelTxt-v1.5（保存标注文件）
"""
...
raw_data = 'F:/code/DATA/'
raw_images_dir = os.path.join(raw_data, 'xin')
raw_label_dir = os.path.join(raw_data, 'DOTA15')

save_dir = 'F:/code/DATA/DOTA_clip3/'
...
for idx, img in enumerate(images):
    ...
    clip_image(img.strip('.jpg'), img_data, box, 608, 608)
